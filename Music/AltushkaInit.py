...
class AltushkaInit:
    ...
    def getBot(self) -> AltushkaBot:
        return self.__bot

    def __create_bot(self, willListen: bool) -> AltushkaBot:
        if willListen:
            prefix = self.__config.BOT_PREFIX
            bot = AltushkaBot(listingSlash=True,
            ...
            bot = AltushkaBot(listingSlash=False,
                            command_prefix=prefix,
                            pm_help=True,
                            case_insensitive=True,
                            intents=self.__intents)
    ...
    def __add_cogs(self, bot: Bot) -> None:
        try:
            ...
            if len(bot.cogs.keys()) != self.__getTotalCogs():
                print(cogsStatus)
                raise AltError(message='Failed to load some Cog')

        except AltError as e:
            print(f'[Error Loading Altushka!]')
            print(e)
