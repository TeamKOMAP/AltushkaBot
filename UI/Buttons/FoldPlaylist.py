...
class FoldPlaylist(Select, AbstractItem):
    """Получает n элементов для размещения в раскрывающемся списке и возврата выбранных, передает значение индекса обработчику."""

    def __init__(self, bot: AltushkaBot, handler: type[AbstractHandler], playlist: Playlist, textChannel: TextChannel, guildID: int, category: MessagesCategory):
    ...
    async def callback(self, interaction: Interaction) -> None:
        ...
        # Очистите последнее отправленное сообщение в этой категории и добавьте новое.
        if message:
            await self.__messagesManager.addMessageAndClearPrevious(self.__guildID, self.__category, message, response.view)

        # Чрезвычайно уродливый способ дождаться, пока процесс проигрывателя действительно получит следующую песню
    ...
    async def __update(self):
        ...
        values = [str(x) for x in range(1, len(songs) + 1)]
        # Получите название каждой из 20 первых песен, больше библиотека не принимает
