...
class HandlerButtons(Button):
    """Кнопка, которая создаст и выполнит объект-обработчик при нажатии"""

    def __init__(self, bot: AltushkaBot, handler: type[AbstractHandler], emoji: AEmojis, textChannel: TextChannel, guildID: int, category: MessagesCategory, label=None, *args, **kwargs):
