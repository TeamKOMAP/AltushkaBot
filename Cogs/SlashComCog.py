...
from Messages.Replies.SlashEmbedReply import SlashEmbedReply
...
class SlashCommands(Cog):
    """
    Класс для обработки музыкальных команд
    При срабатывании создаст специальный обработчик для команды.
    При завершении работы обработчика отобразит созданное в Discord.
    """
    ...
    async def play(self, ctx: ApplicationContext,
                   music: Option(str, "Music name or URL", required=True)) -> None:
        ...
        try:
            ...
            if response is not None:
                cogResponser1 = SlashEmbedReply(response, ctx, MessagesCategory.PLAYER)
    ...
    async def queue(self, ctx: ApplicationContext,
        ...
        try:
            ...
            cogResponser = SlashEmbedReply(response, ctx, MessagesCategory.QUEUE)
    ...
    async def skip(self, ctx: ApplicationContext) -> None:
        ...
        try:
            ...
            cogResponser = SlashEmbedReply(response, ctx, MessagesCategory.PLAYER)
            await cogResponser.run()
        except Exception:
            print(f'[ERROR IN SLASH COMMAND] -> {traceback.format_exc()}')
    ...
    async def history(self, ctx: ApplicationContext) -> None:
        ...
        try:
            ...
            response = await controller.run()
            cogResponser = SlashEmbedReply(response, ctx, MessagesCategory.HISTORY)
            await cogResponser.run()
        except Exception:
            print(f'[ERROR IN SLASH COMMAND] -> {traceback.format_exc()}')
    ...
    async def loop(self, ctx: ApplicationContext,
        ...
        try:
            ...
            cogResponser = SlashEmbedReply(response, ctx, MessagesCategory.LOOP)
            await cogResponser.run()
        except Exception:
            print(f'[ERROR IN SLASH COMMAND] -> {traceback.format_exc()}')
    ...
    async def now_playing(self, ctx: ApplicationContext) -> None:
        ...
        try:
            ...
            response = await controller.run()
            cogResponser = SlashEmbedReply(response, ctx, MessagesCategory.NOW_PLAYING)
            await cogResponser.run()
        except Exception:
            print(f'[ERROR IN SLASH COMMAND] -> {traceback.format_exc()}')
    ...
    async def move(self, ctx: ApplicationContext,
        ...
        try:
            ...
            cogResponser = SlashEmbedReply(response, ctx, MessagesCategory.MANAGING_QUEUE)
            await cogResponser.run()
        except Exception:
            print(f'[ERROR IN SLASH COMMAND] -> {traceback.format_exc()}')
