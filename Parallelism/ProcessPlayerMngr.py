...
class PlayerProcessInfo:
    """Класс для хранения ссылок на все структуры для поддержки проигрывателя процессов"""
...
class ProcessPlayerManager(Singleton, AbstractPlayersManager):
    """
    Управляйте всеми запущенными процессами проигрывателя, создавая и сохраняя их для будущих вызовов.
    Занимается созданием общей памяти
    """
    ...
    def resetPlayer(self, guild: Guild, context: Context) -> None:
        ...
        playCommand = ACommands(ACommandsType.PLAY)
    ...
    def __putCommandInQueue(self, queue: Queue, command: ACommands) -> None:
        try:
            queue.put(command)
        except Exception as e:
