...
class TimeoutClock:
    ...
    async def __executor(self):
        await asyncio.sleep(AConfigs().VC_TIMEOUT)
...
class ThreadPlayer(Thread):
    ...
    def __init__(self, bot: AltushkaBot, guild: Guild, name: str, voiceChannel: VoiceChannel, playlist: Playlist, lock: Lock, guildID: int, voiceID: int, callbackToSendCommand: Callable, exitCB: Callable) -> None:
    ...
    async def __playSong(self, song: Song) -> None:
        ...
        try:
            ...
            nowPlayingCommand = ACommands(ACommandsType.NOW_PLAYING, song)
    ...
    async def receiveCommand(self, command: ACommands) -> None:
        try:
            self.__playerLock.acquire()
            type = command.getType()
            args = command.getArgs()

            if type == ACommandsType.PAUSE:
                self.__pause()
            elif type == ACommandsType.RESUME:
                await self.__resume()
            elif type == ACommandsType.SKIP:
                await self.__skip()
            elif type == ACommandsType.PLAY:
                await self.__playPlaylistSongs()
            elif type == ACommandsType.PREV:
                await self.__playPrev(args)
            elif type == ACommandsType.RESET:
                await self.__reset()
            elif type == ACommandsType.STOP:
                await self.__stop()
            elif type == ACommandsType.VOLUME:
    ...
    async def __timeoutHandler(self) -> None:
        try:
            ...
            if not AConfigs().SHOULD_AUTO_DISCONNECT_WHEN_ALONE:
                return
