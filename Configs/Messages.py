...
class Messages(Singleton):
    def __init__(self) -> None:
        if not super().created:
            ...
            self.SONGINFO_DURATION = "Длительность: "
            self.SONGINFO_REQUESTER = 'Заказчик: '
...
class SearchMessages(Singleton):
    def __init__(self) -> None:
        if not super().created:
            config = AConfigs()
            self.UNKNOWN_INPUT = f'Странный тип ввода, попробуй по-другому или используй {config.BOT_PREFIX}help для полного списка команд'
