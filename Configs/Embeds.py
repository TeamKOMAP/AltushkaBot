...
from Configs.Colors import AColors
...
class AEmbeds:
    ...
    def __willShowProject(self) -> bool:
        return (random() * 10 < self.__config.CHANCE_SHOW_PROJECT)

    def __addFooterContent(self, embed: Embed) -> Embed:
        footerText = f'\u200b Ссылка на проект: {self.__config.PROJECT_URL}'
    ...
    def SONG_INFO(self, info: dict, title: str, position='Воспроизводится') -> Embed:
        embedvc = Embed(
            title=title,
    ...
    def CUSTOM_ERROR(self, error: AltError) -> Embed:
        embed = Embed(
            title=error.title,
    ...
    def RANDOM_NUMBER(self, a: int, b: int, x: int) -> Embed:
        embed = Embed(
            title=f'Случайное число из - [{a}, {b}]',
            description=x,
    ...
    def KOMARU_KOMUGI(self, result: str) -> Embed:
        embed = Embed(
            title='Komaru, Komugi',
    ...
    def BAD_CHOOSE_USE(self) -> Embed:
        embed = Embed(
            title='Выберите что-нибудь',
            description=f'Error: Используйте {self.__config.BOT_PREFIX}help для полного списка команд.',
            colour=self.__colors.RED
        )
        return embed
